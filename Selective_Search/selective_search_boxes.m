...
% Parameters. Note that this controls the number of hierarchical segmentations which are combined.
...
% After segmentation, filter out boxes which have a width/height smaller than minBoxWidth (default = 20 pixels).
minBoxWidth = 20;
...
for j = 1:length(ks)
  ...
  for n = 1:length(colorTypes)
    ...
    [boxesT{idx},~,~,~,priorityT{idx}] = Image2HierarchicalGrouping(im, sigma, k, minSize, colorType, simFunctionHandles);
